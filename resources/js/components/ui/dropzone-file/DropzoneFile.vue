...
function removeFile(index: number) {
	...
	errorMessage.value = ""; // remove erro se houver
...
<template>
	<div class="relative flex flex-col items-center justify-center w-full min-h-64
           ...
           dark:hover:bg-white/10 transition p-4" @click="triggerFileInput">
		<!-- SE EXISTEM ARQUIVOS -->
		<div v-if="filesPreview.length > 0" class="grid grid-cols-3 gap-3 w-full">
			...
			<div v-for="(item, index) in filesPreview" :key="index"
				...
				<!-- Botão remover -->
				<button class="dropzone-remove-btn absolute top-1 right-1 bg-black/60 text-white p-1 rounded-full"
					@click.stop.prevent="removeFile(index)">
					<component :is="getIcon('X')" class="w-4 h-4" />
				</button>

				<img v-if="item.url" :src="item.url" class="w-24 h-24 object-cover rounded-md" />
		...
		<input ref="fileInput" type="file" class="hidden" :multiple="multiple" :accept="acceptString"
			@change="handleFiles" />
	</div>
</template>
