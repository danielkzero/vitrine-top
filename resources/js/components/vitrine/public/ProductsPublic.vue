...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900">
    ...
    <header class="px-4 pt-6 pb-4">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1 relative">
          <button class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><component :is="getIcon('Search')" class="w-4 h-4" /></button>
          <input
            v-model="q"
            type="search"
        ...
        <button @click="viewMode = viewMode === 'grid' ? 'list' : 'grid'" class="p-2 bg-white rounded-xl shadow-sm border border-slate-100 ml-2">
      ...
      <div class="mt-3 overflow-x-auto pb-2">
        <div class="flex gap-2">
          <button
            :class="['px-3 py-1.5 rounded-full text-sm whitespace-nowrap', !selectedCategory ? 'bg-sky-500 text-white' : 'bg-white border']"
            @click="selectedCategory = null"
          >Todas</button>

          <button
            v-for="c in categories" :key="c.id"
            :class="['px-3 py-1.5 rounded-full text-sm whitespace-nowrap', selectedCategory === c.id ? 'bg-sky-500 text-white' : 'bg-white border']"
    ...
    <section v-if="featured.length" class="px-4 pb-4">
      <div class="flex gap-3 overflow-x-auto pb-2">
        <div v-for="f in featured" :key="f.id" class="min-w-[220px] bg-gradient-to-tr from-sky-400 to-emerald-500 text-white rounded-xl p-4 shadow-lg cursor-pointer"
          @click="openProduct(f)">
          <div class="flex items-center gap-3">
            <div class="flex-1">
              <h4 class="font-bold text-lg line-clamp-2">{{ f.name }}</h4>
              <p class="text-sm opacity-90 mt-1">R$ {{ Number(f.discount_price || f.price).toFixed(2) }}</p>
            </div>
            <img v-if="f.images?.length" :src="f.images[0].image_base64 || ('/' + f.images[0].image_path)" class="w-20 h-20 object-cover rounded-lg" loading="lazy" />
          </div>
        </div>
      </div>
    </section>
    ...
    <main class="px-4 pb-32">
      ...
      <div v-if="viewMode === 'grid'" class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      <div v-else class="space-y-3">
        <div v-for="p in visibleProducts" :key="p.id" class="bg-white rounded-xl p-3 shadow-sm flex gap-3 items-center cursor-pointer" @click="openProduct(p)">
          <img v-if="p.images?.length" :src="p.images[0].image_base64 || ('/' + p.images[0].image_path)" class="w-20 h-20 object-cover rounded-lg" loading="lazy" />
          <div class="flex-1">
            <div class="flex justify-between items-start">
              <h4 class="font-bold text-sm">{{ p.name }}</h4>
              <div class="text-emerald-600 font-bold">R$ {{ Number(p.discount_price || p.price).toFixed(2) }}</div>
            </div>
    ...
    <!-- Bottom nav + WhatsApp -->
    <BottomNav :pages="/* seu array de pages ou ícones */ []" />
