...
<template>
  ...
  <AppLayout :breadcrumbs="[{ title: 'Início', href: routes.painel.index }, { title: 'Páginas', href: '#' }]">
      ...
      <div>
        ...
        <div v-if="pages.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          <div v-for="page in pages" :key="page.id"
            class="bg-white border rounded-xl p-4 shadow-sm hover:shadow-md transition flex flex-col justify-between">
            <div>
              <!-- Ícone + Título -->
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                  <component :is="page.icon ? LucideIcons[page.icon] : Globe" class="w-5 h-5 text-sky-500" />
            ...
            <div class="flex justify-between items-center mt-4">
              <p class="text-gray-600 text-sm line-clamp-3 bg-amber-300 p-2 px-4 rounded-full">
